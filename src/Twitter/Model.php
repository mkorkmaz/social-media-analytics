...
class Model
{
    ...
    public function processLatestTweets(string $user_id, array $tweets)
    {
        ...
        foreach ($tweets as $tweet) {
            ...
            $is_exists = $this->db->find('posts', ['post_id' => $tweet['id_str'], 'post_type' => 'tweet'], ['id'], null, 0, 1);
            $doc = [
                'timestamp' => (int) $now,
    ...
    public function updateTweetData()
    {
        ...
        $aMonthAgo = $aMonthAgoDate->getTimestamp()*100;
        $tweets = $this->db->find('posts', ['is_active' => 1, 'timestamp__gte' => $aMonthAgo], ['post_id'], null, 0 , 10000);
        $idsBucket=[];
        $bucketIndex = 0;
        ...
        foreach ($idsBucket as $idBucket) {
            ...
            foreach ($lookupTweets->id as $id_str => $tweet) {
                if (empty($tweet)) {
                    $this->db->update('posts', ['post_id' => $id_str, 'post_type' => 'tweet'], ['is_active' => 0]);
                } else {
                    $doc = [
